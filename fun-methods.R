...
funmeth.mlp = function(data, drug){
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~Aquesta part la vam fer servir per fixar la size i el decay ~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # N <- nrow(df)
    # learn <- sample(1:N, round(0.9*N))
    # nlearn <- length(learn)
    # ntest <- N - nlearn
    ...
    #Per una execució més ràpida i que es pugui testejar amb fixat la size i el decay perque sino tarda hores.
